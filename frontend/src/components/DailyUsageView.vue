...
const maxTokens = computed(() => Math.max(1, ...items.value.map(i => Number(i.totalTokens) || 0)))
...
async function search() {
  ...
  error.value = ''
  try {
    result.value = await api.dailyStats({
...
function barWidth(v) {
  return `${Math.max(1.5, (Number(v) || 0) / maxTokens.value * 100)}%`
}
function fmtInt(n) {
...
<template>
  <div>
    <div class="flex items-center justify-between mb-3">
      <h2 class="text-base font-semibold"><i class="fa-solid fa-chart-line mr-1.5 text-gh-blue"></i>每日用量统计</h2>
      <span class="text-xs text-gh-muted">按日期统计所选区间内的 token 消耗（无调用的日期补零）</span>
    </div>
      ...
      <table class="w-full min-w-[960px] text-sm">
        <thead>
          <tr class="border-b border-gh-border bg-gh-tag/60 text-left text-xs text-gh-muted">
            ...
            <th class="px-3 py-2 w-44">Token 趋势</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-for="i in items" :key="i.date" class="border-b border-gh-border/40 hover:bg-gh-cyan/5 transition-colors">
            <td class="px-3 py-1.5 font-mono text-xs">{{ i.date }}</td>
          ...
          <tr v-if="items.length > 0" class="bg-gh-cyan/[0.06] font-medium border-t border-gh-cyan/20">
            ...
            <td class="px-3 py-2 text-right font-mono text-xs" :class="Number(total.errors) > 0 ? 'text-gh-red font-semibold' : 'text-gh-muted'">
              {{ fmtInt(total.errors) }}
            </td>
            <td class="px-3 py-2" />
          </tr>
